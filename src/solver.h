...
public:
  ...
  ~sp_node() {}
  bool operator==(sp_node<T> tmp) { return idx == tmp.idx; }
  bool operator==(const sp_node<T> &tmp) const { return idx == tmp.idx; }
  T val;
  size_t idx;
};

template <typename T> class sp_vec : public std::vector<sp_node<T>> {
public:
  ...
  T operator[](size_t i) const {
    const_iterator it = std::find(this->begin(), this->end(), sp_node<T>(i));
    if (it == this->end())
      return T(0);
    else
      return it->val;
...
template <typename T> std::ostream &operator<<(std::ostream &out, sp_vec<T> v) {
  out << "sparse vector\n";
...
template <typename T> class sp_mat : public std::vector<sp_vec<T>> {
public:
...
template <typename T> std::ostream &operator<<(std::ostream &out, sp_mat<T> v) {
  out << "sparse matrix\n";
...
extern "C" void zneupd_(int *rvec, char *All, int *select,
                        std::complex<double> *d, std::complex<double> *v,
                        int *ldv, double *sigma, std::complex<double> *workev,
...
protected:
  ...
  void solve_eig_zmumps(sp_mat<std::complex<double>> &a,
                        sp_mat<std::complex<double>> &b, size_t &n_t,
  ...
  get_jac(std::vector<std::vector<double>> geo);
  std::vector<std::vector<double>> get_shape(type s, type e, type o,
